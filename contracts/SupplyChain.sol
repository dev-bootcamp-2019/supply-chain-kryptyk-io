...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
    ...
    modifier verifyCaller (address _address) {
        require (msg.sender == _address, "Identity doesn't match.");
        _;
    }
  ...
  /* For each of the following modifiers, use what you learned about modifiers
   ...
   that the item with the given sku has the state ForSale. */
    modifier forSale(uint sku) {
        require(items[sku].state == State.ForSale, "This item is not for sale.");
        _;
    }
    modifier sold(uint sku) {
        require(items[sku].state == State.Sold, "This item is not sold.");
        _;
    } 
    modifier shipped(uint sku) {
        require(items[sku].state == State.Shipped, "This ityem is not shipped.");
        _;
    }
    modifier received(uint sku) {
        require(items[sku].state == State.Received, "This item is not received.");
        _;
    }
